                        ...
                        <form method="POST" action="{{ route('tickets.update', $ticket->id) }}">
                            ...
                            <div class="form-group">
                                ...
                                <input name="title" type="text" value="{{ old('title', $ticket->title) }}"
                                    class="form-control" id="title" placeholder="Digite o título do chamado">
                            </div>
                            @error('title')
                            ...
                            <div class="form-group">
                                ...
                                <textarea name="description" class="form-control" id="description" rows="4"
                                    placeholder="Descreva o seu problema.">{{ old('description', $ticket->description) }}</textarea>
                            ...
                            <div class="form-group">
                                ...
                                <select name="status" class="form-control" id="status">
                                    @foreach ($status as $key)
                                        <option value="{{ $key }}"
                                            {{ old('status', $ticket->status) == $key ? 'selected' : '' }}>
                                            {{ $key }}</option>
                                    @endforeach
                                </select>
                                @error('status')
